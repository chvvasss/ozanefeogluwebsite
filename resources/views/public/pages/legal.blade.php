...
@php
    ...
    $eyebrow      = $page->extra('eyebrow', 'Hukuk');
    $contactEmail = site_setting('contact.email');
@endphp
    ...
    <section class="scene scene--muted border-t border-[var(--color-rule)]">
        <div class="page-wrap-narrow">
            <div class="prose-article">
                ...
                <p><small>
                    Son güncellenme: {{ $page->updated_at?->translatedFormat('d F Y') }}.
                    @if ($contactEmail)
                        İletişim: <a href="mailto:{{ $contactEmail }}">{{ $contactEmail }}</a>.
                    @endif
                </small></p>
            </div>
        </div>
